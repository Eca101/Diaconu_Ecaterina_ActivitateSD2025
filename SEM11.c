...
void inserareInLS(NodLS** cap, NodLP* nodPrincipal)//ad la incepur
{
...
void inserareMuchie(NodLP* listaPrincipala, int idStart, int idStop) 
{
	...
	if (nodStart != NULL && nodStop != NULL)
	{
		...
		inserareInLS(&(nodStop->vecini), nodStart);//DOAR PT GRAF NEORIENTAT
	}
}
...
NodLP* citireGraf(const char* fisierMasini, const char* fisierMuchii)
{
	NodLP* graf = citireNoduriMasiniDinFisier("masini.txt");
